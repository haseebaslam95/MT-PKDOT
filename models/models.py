...
class PHY_MODEL(nn.Module):
    '''
    CNN-LSTM model.
    
    '''
    def __init__(self):
        super(PHY_MODEL,self).__init__()
    ...
    def model_out(self,video_batch,specs_2d):
        ...
        # combined_out=torch.cat((vis_out,phy_out),1)
        output = self.out_layer1(phy_out)
        # output = self.out_layer2(output)
        output = self.out_layer3(output)
        return output
...
# create a small classifer that take in feature vector and predicts class
